...
export function Header() {
  ...
  function submit(event: FormEvent<HTMLFormElement>) {
    ...
    if (value) router.push(`/dashboard/arquivos?busca=${encodeURIComponent(value)}`);
  }
  ...
  return (
    <header className="z-10 flex h-16 flex-shrink-0 items-center justify-between border-b border-[#E2E8F0] bg-white px-4 md:px-6">
      ...
      <div className="flex items-center gap-4">
        ...
        <Link href="/dashboard/configuracoes" className="flex h-8 w-8 items-center justify-center rounded-full border border-[#4F46E5]/20 bg-[#4F46E5]/10 text-sm font-bold text-[#4F46E5]" aria-label="Configuracoes">
          AE
        </Link>
      </div>
    </header>
