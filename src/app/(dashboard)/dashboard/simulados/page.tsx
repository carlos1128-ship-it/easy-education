  ...
  return (
    <div className="mx-auto max-w-6xl space-y-6">
      <div>
        ...
        <h1 className="mt-2 text-3xl font-bold tracking-tight text-[#0F172A]">Pratica em ritmo de prova</h1>
      </div>
      ...
      <section className="grid gap-4 md:grid-cols-3">
        {[
          ...
          { label: "Media geral", value: `${average}%`, icon: Target },
          { label: "Questoes geradas", value: String(simulados.reduce((sum, quiz) => sum + quiz.questionCount, 0)), icon: Clock },
      ...
      <section className="rounded-[20px] border border-[#E2E8F0] bg-white shadow-sm">
        ...
        <div className="divide-y divide-[#F1F5F9]">
          {simulados.length ? simulados.map((exam) => (
            <Link key={exam.id} href={`/dashboard/quizzes/${exam.id}`} className="flex flex-col gap-3 p-4 transition-colors hover:bg-[#F8FAFC] sm:flex-row sm:items-center">
              ...
              <div className="flex-1">
                ...
                <p className="text-sm text-[#64748B]">{exam.questionCount} questoes</p>
              </div>
              <span className="w-fit rounded-md bg-[#EEF2FF] px-2.5 py-1 text-xs font-bold text-[#4F46E5]">
                {exam.completedAt ? "Concluido" : "Disponivel"}
              </span>
              <span className="text-sm font-semibold text-[#64748B]">{exam.score === null ? "Nao iniciado" : `${Math.round(exam.score)}%`}</span>
            </Link>
          )) : (
